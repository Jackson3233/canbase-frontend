...
export function WaitlistTable<TData, TValue>({
  columns,
  data,
}: DataTableProps<TData, TValue>) {
  const [waitlistTableSort, setWaitlistTableSort] = useLocalStorage(
    "waitlist-table-sort",
    []
  );
  const [waitlistTableShow, setWaitlistTableShow] = useLocalStorage(
    "waitlist-table-show",
    {}
  );
  const [waitlistTableSize, setWaitlistTableSize] = useLocalStorage(
    "waitlist-table-size",
    initialColumnSizing
  );
  ...
  const [columnSizing, setColumnSizing] = useState<ColumnSizingState>(
    waitlistTableSize || initialColumnSizing
  );
  ...
  const table = useReactTable({
    ...
    state: {
      sorting: waitlistTableSort,
      columnFilters,
      columnVisibility: waitlistTableShow,
      rowSelection,
      columnSizing: waitlistTableSize,
    },
  });

  useEffect(() => {
    setWaitlistTableSort(mergeObjectArrays(waitlistTableSort, sorting) as any);
  }, [sorting]);

  useEffect(() => {
    setWaitlistTableShow(mergeObjects(waitlistTableShow, columnVisibility));
  }, [columnVisibility]);

  useEffect(() => {
    setWaitlistTableSize(mergeObjects(waitlistTableSize, columnSizing));
  }, [columnSizing]);

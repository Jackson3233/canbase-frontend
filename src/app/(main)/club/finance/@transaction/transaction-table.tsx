...
export function TransactionTable<TData, TValue>({
  ...
  const [transactionTableSort, setTransactionTableSort] = useLocalStorage(
    "transaction-table-sort",
    []
  );
  const [transactionTableShow, setTransactionTableShow] = useLocalStorage(
    "transaction-table-show",
    {}
  );
  const [transactionTableSize, setTransactionTableSize] = useLocalStorage(
  ...
  const [columnSizing, setColumnSizing] = useState<ColumnSizingState>(
    transactionTableSize || initialColumnSizing
  );
  ...
  const table = useReactTable({
    ...
    state: {
      sorting: transactionTableSort,
      columnFilters,
      columnVisibility: transactionTableShow,
      rowSelection,
      columnSizing: transactionTableSize,
    },
  });

  useEffect(() => {
    setTransactionTableSort(
      mergeObjectArrays(transactionTableSort, sorting) as any
    );
  }, [sorting]);

  useEffect(() => {
    setTransactionTableShow(
      mergeObjects(transactionTableShow, columnVisibility)
    );
  }, [columnVisibility]);

  useEffect(() => {
    setTransactionTableSize(mergeObjects(transactionTableSize, columnSizing));
  }, [columnSizing]);
            ...
            {table
              ...
              .map((column) => {
                return (
                  ...
                  >
                    {column.id === "avatar" && "Bild"}
                    {column.id === "recipient" && "Name"}
                    {column.id === "amount" && "Betrag"}
                    {column.id === "method" && "Zahlungsmethode"}
                    {column.id === "purpose" && "Verwendungszweck"}
                    {column.id === "attachments" && "Anhänge"}
                    {column.id === "updatedAt" && "Letztes Update"}
                    {column.id === "createdDate" && "Erstellt"}
                  </DropdownMenuCheckboxItem>
                );
              })}
          </DropdownMenuContent>

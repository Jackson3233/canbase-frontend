...
export function Tabs(props: {
  ...
  return (
    <TabsContext.Provider value={{ ...other, hrefFor, searchParam, selected, pathname }}>
...
export function TabsList(props: {
  ...
  return (
    <div
      {...props}
      className={cn(
        "inline-flex h-10 items-center justify-center rounded-b-md bg-muted p-1 text-muted-foreground",
        props.className
      )}
    />
  );
...
export const TabsPage = (props: {
  ...
  return (
    <Link
      ...
      data-state={context.pathname?.includes(props.value) ? "active" : "inactive"}
      href={props.value}
      scroll={false}
      shallow={true}
    />
